...
exports.simplifyError = exports.simplifyCaughtError = exports.shuffleArray = exports.queryEntries = exports.prefixStoragePath = exports.objToUniqueString = exports.oEntries = exports.niceTry = exports.listenReachableServer = exports.getUrlExtension = exports.flatRawEntries = exports.flatEntries = exports.everyEntrie = exports.encryptString = exports.decryptString = exports.cloneInstance = exports.IS_WHOLE_NUMBER = exports.IS_RAW_OBJECT = exports.IS_DECIMAL_NUMBER = void 0;
...
var oEntries = function oEntries(obj) {
  ...
  Object.entries(obj).forEach(function (e) {
    o.push(e);
    if (e[1] && _typeof(e[1]) === 'object' && !Array.isArray(e[1])) {
...
var IS_RAW_OBJECT = function IS_RAW_OBJECT(e) {
  return e && _typeof(e) === 'object' && !Array.isArray(e) && !(e instanceof Date);
};
...
var IS_DECIMAL_NUMBER = function IS_DECIMAL_NUMBER(v) {
  return typeof v === 'number' && "".concat(v).includes('.');
};
exports.IS_DECIMAL_NUMBER = IS_DECIMAL_NUMBER;
var queryEntries = function queryEntries(obj) {
  ...
  var o = [];
  var isArraySeperator = Array.isArray(lastPath);
  Object.entries(obj).forEach(function (_ref) {
    var _ref2 = _slicedToArray(_ref, 2),
      key = _ref2[0],
      value = _ref2[1];
    if (IS_RAW_OBJECT(value) && !exceptions.includes(key)) {
      o = [].concat(_toConsumableArray(o), _toConsumableArray(queryEntries(value, isArraySeperator ? [].concat(_toConsumableArray(lastPath), [key]) : "".concat(lastPath).concat(key).concat(seperator), exceptions, seperator)));
    } else o.push(isArraySeperator ? [[].concat(_toConsumableArray(lastPath), [key]), value] : ["".concat(lastPath).concat(key), value]);
  });
  return o;
};
exports.queryEntries = queryEntries;
var objToUniqueString = function objToUniqueString(obj) {
  var keys = [],
    values = [];
  if (Array.isArray(obj)) {
    obj.forEach(function (e) {
      if (IS_RAW_OBJECT(e)) {
        queryEntries(e).map(function (_ref3) {
      ...
      } else keys.push(Array.isArray(e) ? JSON.stringify(e) : "".concat(e));
    });
  } else if (!IS_RAW_OBJECT(obj)) return "".concat(obj);else queryEntries(obj).map(function (_ref5) {
  ...
  return [].concat(_toConsumableArray(keys.sort()), _toConsumableArray(values.map(function (v) {
    return "".concat(Array.isArray(v) ? JSON.stringify(v) : v);
  }).sort())).join(',');
};
exports.objToUniqueString = objToUniqueString;
var cloneInstance = function cloneInstance(v) {
  if (v && _typeof(v) === 'object') {
    return Array.isArray(v) ? _toConsumableArray(v) : _objectSpread({}, v);
  }
  return v;
};
exports.cloneInstance = cloneInstance;
var listenReachableServer = function listenReachableServer(callback, projectUrl) {
  return _listeners.ServerReachableListener.listenTo(projectUrl, function (t) {
...
var niceTry = function niceTry(promise) {
  return new Promise( /*#__PURE__*/function () {
    var _ref7 = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee(resolve) {
      var r;
...
exports.niceTry = niceTry;
var shuffleArray = function shuffleArray(n) {
  var array = _toConsumableArray(n);
  var currentIndex = array.length,
    randomIndex;
  while (currentIndex != 0) {
    randomIndex = Math.floor(Math.random() * currentIndex);
    currentIndex--;
    var _ref8 = [array[randomIndex], array[currentIndex]];
    array[currentIndex] = _ref8[0];
    array[randomIndex] = _ref8[1];
  }
  return array;
};
...
function sortArrayByObjectKey() {
  ...
  return arr.slice(0).sort(function (a, b) {
    var left = getLodash(a, key),
      right = getLodash(b, key);
    return left > right ? 1 : left < right ? -1 : 0;
  });
}
var encryptString = function encryptString(txt, password, iv) {
  return _cryptoJs.AES.encrypt(txt, "".concat(password || '').concat(iv || '')).toString();
};
exports.encryptString = encryptString;
var decryptString = function decryptString(txt, password, iv) {
  return _cryptoJs.AES.decrypt(txt, "".concat(password || '').concat(iv || '')).toString(_cryptoJs.enc.Utf8);
};
